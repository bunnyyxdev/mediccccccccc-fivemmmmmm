...
ระบบหมอ Preview City ถูกออกแบบให้เป็น **Advanced System** ด้วย features ครบถ้วนและสมบูรณ์
...
```
GET /api/withdraw-items?page=1&limit=10&search=ยา&status=approved&sort=-createdAt
```
...
const response = await fetch('/api/withdraw-items', {
  ...
  body: JSON.stringify({ itemName: 'ยา', quantity: 10 }),
});
```
