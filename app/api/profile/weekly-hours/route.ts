...
async function handlerGET(request: NextRequest, authUser: any) {
  try {
    ...
    const doctorName = dbUser.name || dbUser.username || '';
    
    console.log('[Weekly Hours API] Fetching hours for user:', {
      userId: authUser.userId,
      name: dbUser.name,
      username: dbUser.username,
      usingName: doctorName,
    });
    ...
    console.log('[Weekly Hours API] Looking for doctor name (normalized):', normalizedDoctorName);
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (rowName) {
        ...
        if (normalizedRowName === normalizedDoctorName) {
    ...
    if (!weeklyHours) {
      console.warn('[Weekly Hours API] No match found for doctor:', {
        originalName: doctorName,
        normalizedName: normalizedDoctorName,
