...
export async function GET(request: NextRequest) {
  try {
    ...
    // Allow both admin and doctor to get user list
    if (!decoded || (decoded.role !== 'admin' && decoded.role !== 'doctor')) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { username, password, name, doctorRank, role } = await request.json();
    ...
    const user = await (User as any).create({
      ...
      name,
      doctorRank: doctorRank || undefined,
      role: role || 'doctor',
    });
    ...
    try {
      ...
      if (doctorRank) {
        discordMessage += `**ยศแพทย์:** ${doctorRank}\n`;
      }
      discordMessage += `**บทบาท:** ${role === 'admin' ? 'ผู้ดูแลระบบ' : 'แพทย์'}\n`;
