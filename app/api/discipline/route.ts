...
async function handlerPOST(request: NextRequest, user: any) {
  try {
    ...
    if (!body.doctorName || !body.violation) {
      return NextResponse.json(
        { error: 'กรุณากรอกข้อมูลให้ครบถ้วน (ชื่อแพทย์, ความผิด)' },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      await logActivity({
        ...
        entityName: `โทษวินัย: ${record.doctorName}`,
    ...
    try {
      let discordMessage = `**ชื่อแพทย์:** ${record.doctorName}\n`;
